    ...
    return (
        <>
            <List>
                ...
                <ListItem>
                    Назначение: <TextField disabled={boxes !== undefined && boxes?.find((v) => v.boxNumber == item.boxNum)?.isPacked !== false} error={item.appointment.trim() == ""} required sx={{ml: '1rem', width: "auto"}} type='text' value={item.appointment}
                                                onChange={e => {
